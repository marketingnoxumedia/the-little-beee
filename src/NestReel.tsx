...
// ---------------------------------------------------------------------------
// "The withdrawal rate" reel (NestReel) — the retirement safe-withdrawal-rate rule, told
// through the numbers. This is an A/B TWIN of SpeedReel: same message, structure and captions,
// its OWN fresh footage (public/nest/), and a different hook opener ("Want to know…" vs "Did
// you ever wonder…"). Two people retire with the exact same $1,000,000; one runs out, the other
// never does — same balance, same markets, the only difference is how fast they spend. One
// draws $40,000 a year (4%, a rate that has historically tended to last decades), the other
// $80,000 (8%, double the income and a far higher chance of running dry). It isn't about better
// investments — same portfolio, same returns; double the withdrawal rate and you roughly double
// the risk of running out, and timing sharpens it (a bad market early hurts the fast spender
// most). The balance was never the whole game — the rate is the hidden variable. Numbers-driven:
// four count-up `stat` beats ($40k, $80k, 4%, 8%). A short ~1.5s pause after the hook (house
// convention), black beats for the punch-lines, one red "the rate is the hidden variable"
// `impact` card as the mid-tail thesis, and a rhetorical question close ("which retiree were
// you?") on footage rather than the red card. Durations on exact spoken-word timestamps
// (ElevenLabs alignment) at +6% pace. Total 1568 frames = ~52.3s. `enter` adds a transition on
// some cuts. Every scene uses a distinct background sourced fresh for THIS reel (public/nest/) —
// no footage shared with SpeedReel, DrawReel, or any other reel (hard rule). NOTE: this is the
// SAME topic as the existing DrawReel ("The rate, not the balance") AND SpeedReel — publish only
// one, or run the twin as a deliberate A/B against SpeedReel. FLAG: illustrative — the "4% lasts
// decades / 8% far higher risk" figures are the widely-cited safe-withdrawal rule of thumb, not
// a guarantee; actual outcomes depend on returns, sequence, horizon and fees. Verify the framing
// before publishing. Imagery is generic (no fund/brokerage brands, no legible tickers, no
// recognizable people).
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 117, kind: 'hook', text: 'Two retire,|same million.', kicker: 'The withdrawal rate', highlights: ['million'], size: 88, media: {src: 'nest/v_hook.mp4', type: 'video', effect: 'in'}},
  // Short ~1.5s pause after the hook (house convention): 45 frames of silence are spliced
  // into the VO right after "...the other never does?", and this beat holds through it.
  {dur: 105, kind: 'text', text: 'One runs out.|One never does.', highlights: ['never'], size: 88, media: {src: 'nest/p_nest.jpg', type: 'img', effect: 'in'}},
  {dur: 75, kind: 'text', text: 'Same balance.|Same markets.', highlights: ['markets'], size: 90},
  {dur: 51, kind: 'stat', stat: {pre: 'ONE DRAWS', prefix: '$', value: 40000, post: 'A YEAR'}, media: {src: 'nest/p_cash.jpg', type: 'img', effect: 'in'}},
  {dur: 43, kind: 'stat', stat: {pre: 'THE OTHER DRAWS', prefix: '$', value: 80000, post: 'A YEAR'}},
  {dur: 81, kind: 'text', enter: 'slideL', text: 'You\'d think the pot|decides it.', highlights: ['pot'], size: 86, media: {src: 'nest/p_relaxed.jpg', type: 'img', effect: 'in'}},
  {dur: 62, kind: 'text', text: 'But the rate|sets the odds.', highlights: ['rate'], size: 90, media: {src: 'nest/p_calc.jpg', type: 'img', effect: 'in'}},
  {dur: 106, kind: 'stat', stat: {pre: 'DRAW', value: 4, suffix: '%'}, media: {src: 'nest/p_chart.jpg', type: 'img', effect: 'in'}},
  {dur: 33, kind: 'text', text: 'Historically,|lasts decades.', highlights: ['decades'], size: 90},
  {dur: 132, kind: 'stat', stat: {pre: 'OR DRAW', value: 8, suffix: '%'}, media: {src: 'nest/p_worried.jpg', type: 'img', effect: 'in'}},
  {dur: 44, kind: 'text', text: 'A far higher chance|of running dry.', highlights: ['dry'], size: 84, media: {src: 'nest/p_wallet.jpg', type: 'img', effect: 'in'}},
  {dur: 61, kind: 'text', text: 'Not about better|investments.', highlights: ['investments'], size: 88},
  {dur: 85, kind: 'text', text: 'Same portfolio,|same returns.', highlights: ['returns'], size: 88},
  {dur: 88, kind: 'text', enter: 'slideL', text: 'Double the rate,|double the risk.', highlights: ['double'], size: 88},
  {dur: 93, kind: 'text', text: 'A bad market early|hurts most.', highlights: ['early'], size: 86, media: {src: 'nest/p_crash.jpg', type: 'img', effect: 'in'}},
  {dur: 72, kind: 'text', enter: 'slideL', text: 'The balance was never|the whole game.', highlights: ['balance'], size: 82, media: {src: 'nest/v_drain.mp4', type: 'video', effect: 'in'}},
  {dur: 69, kind: 'impact', text: 'The rate is the|hidden variable.', redBg: true},
  {dur: 95, kind: 'text', enter: 'slideL', text: 'Two speeds,|two endings.', highlights: ['speeds'], size: 92, media: {src: 'nest/p_paths.jpg', type: 'img', effect: 'in'}},
  {dur: 107, kind: 'text', text: 'Only the speed|differs.', highlights: ['speed'], size: 92},
  {dur: 49, kind: 'text', enter: 'zoom', text: 'Which retiree|were you?', highlights: ['retiree'], size: 92, media: {src: 'nest/v_close.mp4', type: 'video', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
// NOTE: a 45-frame (~1.5s) pause is spliced after the hook (at frame ~177), so cues at or
// after the reveal (frame 222) are already on the post-pause timeline.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 222, src: 'media/sfx_whoosh.mp3', vol: 0.4},   // reveal after the pause
  {at: 297, src: 'media/sfx_impact.mp3', vol: 0.5},   // $40k stat
  {at: 348, src: 'media/sfx_impact.mp3', vol: 0.5},   // $80k stat
  {at: 534, src: 'media/sfx_impact.mp3', vol: 0.45},  // 4% stat
  {at: 673, src: 'media/sfx_impact.mp3', vol: 0.5},   // 8% stat
  {at: 1248, src: 'media/sfx_impact.mp3', vol: 0.72}, // red thesis card
  {at: 1519, src: 'media/sfx_whoosh.mp3', vol: 0.4},  // close
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The withdrawal rate</span>
      </div>
...
// ---------------------------------------------------------------------------
export const NestReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('nest/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('nest/music.mp3')}
