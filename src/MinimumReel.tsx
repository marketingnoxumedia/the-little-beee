...
// ---------------------------------------------------------------------------
// "The minimum trap" reel (MinimumReel) — why a $5,000 credit-card balance can
// take 15+ years to clear: at ~21% APR, paying only the minimum, you pay roughly
// the balance again in interest. Switch to a fixed $250/month and the same debt
// clears in ~2 years with ~$1,100 interest — the only change is a fixed payment
// instead of the minimum. The minimum is built to keep the account current while
// the principal barely moves: solvent and indebted at once. Interest runs on the
// balance, so the faster the balance falls the less it all costs. Question hook
// ("$5,000 on a card — over 15 years to clear?") with a ~1.2s post-hook pause,
// red `impact` card ("Solvent. / And indebted.") mid-tail, and a question close
// ("15 years of this — paying for what?"). Durations on exact ElevenLabs
// spoken-word timestamps at +6% pace; the pause is spliced into the VO right
// after the hook question ("…to clear?") and beat 1 holds through it (115 ->
// 151). Total 1656 frames = ~55.2s. **Media-dense:** 18 of 21 beats carry
// footage, with only 2 non-media beats (a black "Not a plan. / A trap." impact
// and a black "The principal barely moves.") plus the one red card. Own footage
// under public/minimum/ — no asset shared with any other reel (skill hard rule).
// **Unverified figures — verify before publishing:** the $1.25T total US
// card debt and ~21% average APR track recent reporting, and the amortization
// figures ($5K at ~21% ≈ 15+ yrs / ~$5K interest on the minimum vs ~2 yrs /
// ~$1,100 on a fixed $250) are standard minimum-payment math on stated
// assumptions — sanity-check the specifics (rate, minimum formula) before
// publishing.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 92, kind: 'hook', text: '$5,000 on|a card.', kicker: 'The minimum trap', highlights: ['card'], size: 92, media: {src: 'minimum/p_card.jpg', type: 'img', effect: 'in'}},
  // 115 -> 151: a 36-frame (~1.2s) pause is spliced into the VO right after the
  // hook question ("…take over 15 years to clear?"), and this beat holds through it.
  {dur: 151, kind: 'text', text: 'Over 15 years|to clear?', highlights: ['15'], size: 88, media: {src: 'minimum/p_calendar.jpg', type: 'img', effect: 'in'}},
  {dur: 62, kind: 'text', text: 'Pay only|the minimum.', highlights: ['minimum'], size: 88, media: {src: 'minimum/p_statement.jpg', type: 'img', effect: 'in'}},
  {dur: 84, kind: 'text', text: 'You pay the balance|again in interest.', highlights: ['again'], size: 76, media: {src: 'minimum/p_interestup.jpg', type: 'img', effect: 'in'}},
  {dur: 118, kind: 'stat', stat: {pre: 'Americans owe', prefix: '$', value: 1.25, decimals: 2, suffix: 'T'}, media: {src: 'minimum/p_debtpile.jpg', type: 'img', effect: 'in'}},
  {dur: 81, kind: 'stat', stat: {pre: 'Average rate', value: 21, suffix: '%', bar: 21}, media: {src: 'minimum/p_gauge.jpg', type: 'img', effect: 'in'}},
  {dur: 55, kind: 'impact', text: 'Not a plan.|A trap.', redBg: false},
  {dur: 109, kind: 'stat', stat: {pre: 'Carry', prefix: '$', value: 5000}, media: {src: 'minimum/p_5kcash.jpg', type: 'img', effect: 'in'}},
  {dur: 72, kind: 'stat', stat: {pre: 'Minimum only', value: 15, post: 'years'}, media: {src: 'minimum/v_hourglass.mp4', type: 'video', effect: 'in'}},
  {dur: 89, kind: 'stat', stat: {pre: 'Interest paid', prefix: '$', value: 5000}, media: {src: 'minimum/p_moneydrain.jpg', type: 'img', effect: 'in'}},
  {dur: 88, kind: 'stat', stat: {pre: 'Or a fixed', prefix: '$', value: 250, post: 'a month'}, media: {src: 'minimum/p_envelope.jpg', type: 'img', effect: 'in'}},
  {dur: 38, kind: 'stat', stat: {pre: 'Clears in', value: 2, post: 'years'}, media: {src: 'minimum/p_paidstamp.jpg', type: 'img', effect: 'in'}},
  {dur: 54, kind: 'stat', stat: {pre: 'Interest, just', prefix: '$', value: 1100}, media: {src: 'minimum/p_fewcoins.jpg', type: 'img', effect: 'in'}},
  {dur: 104, kind: 'text', enter: 'slideL', text: 'Same balance.|One change.', highlights: ['change'], size: 86, media: {src: 'minimum/p_twostacks.jpg', type: 'img', effect: 'in'}},
  {dur: 47, kind: 'text', text: 'It keeps the account|current.', highlights: ['current'], size: 78, media: {src: 'minimum/v_swipe.mp4', type: 'video', effect: 'in'}},
  {dur: 47, kind: 'text', text: 'The principal|barely moves.', highlights: ['barely'], size: 86},
  {dur: 78, kind: 'impact', text: 'Solvent.|And indebted.', redBg: true},
  {dur: 54, kind: 'text', text: 'Interest runs|on the balance.', highlights: ['balance'], size: 82, media: {src: 'minimum/p_growthline.jpg', type: 'img', effect: 'in'}},
  {dur: 113, kind: 'text', text: 'The faster it falls,|the less it costs.', highlights: ['faster'], size: 78, media: {src: 'minimum/p_downchart.jpg', type: 'img', effect: 'in'}},
  {dur: 31, kind: 'text', text: 'Designed|to linger.', highlights: ['linger'], size: 88, media: {src: 'minimum/p_clock.jpg', type: 'img', effect: 'in'}},
  {dur: 89, kind: 'text', enter: 'zoom', text: '15 years of this.|Paying for what?', highlights: ['what?'], size: 78, media: {src: 'minimum/v_scissors.mp4', type: 'video', effect: 'in'}},
];
...
// 92), so every cue below is already on the post-pause timeline.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 243, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 389, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 643, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 824, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 913, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 1291, src: 'media/sfx_impact.mp3', vol: 0.6},
  {at: 1567, src: 'media/sfx_whoosh.mp3', vol: 0.4},
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The minimum trap</span>
      </div>
...
// ---------------------------------------------------------------------------
export const MinimumReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('minimum/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('minimum/music.mp3')}
