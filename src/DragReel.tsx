...
// ---------------------------------------------------------------------------
// "The certain drag" reel (DragReel) — the QUESTION-hook / question-close A/B
// twin of FeeReel (which runs the same fund-fee case declaratively as "The 1%
// tax"). Same math — $100K, 30 years, 7% gross, a 0.05% index fee vs ~1%, a
// ~$180K gap on identical market exposure — but this cut opens on a question
// ("What does a 1% fee cost?") with a ~1.2s post-hook pause, leans on the extra
// angle that the expensive fund probably WON'T even win (~90% of active funds
// trail the index over 15 years), and closes on a question ("What is that 1%
// actually buying?"). Durations on exact ElevenLabs spoken-word timestamps at
// +6% pace; the pause is spliced into the VO right after the hook question
// ("…costs you?") and beat 0 holds through it (107 -> 143). Total 1672 frames =
// ~55.7s. **Media-dense:** 20 of 23 beats carry footage, with only 2 flat-black
// beats ("More than you started with.", "A premium to underperform.") plus one
// red `impact` card ("The fee is certain. / The upside isn't.") as the mid-tail
// thesis; the close lands on footage. Own footage under public/drag/ — no asset
// shared with FeeReel or any other reel (skill hard rule). **Unverified figures
// — verify before publishing:** the $755K / $574K / ~$180K outcomes assume a
// flat 7% gross with the stated fees over 30 years, and "~90% of active funds
// trail the index over 15 years" tracks the SPIVA-style finding — sanity-check
// the specifics. Same topic as FeeReel — publish only one twin to a given
// audience, scheduled apart.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 143, kind: 'hook', text: 'What does a|1% fee cost?', kicker: 'The certain drag', highlights: ['1%'], size: 84, media: {src: 'drag/p_wonder.jpg', type: 'img', effect: 'in'}},
  {dur: 76, kind: 'text', text: 'On $100K,|over 30 years.', highlights: ['30'], size: 84, media: {src: 'drag/p_hundredk.jpg', type: 'img', effect: 'in'}},
  {dur: 56, kind: 'stat', stat: {pre: 'It costs', prefix: '$', value: 180, suffix: 'K'}, media: {src: 'drag/p_leak.jpg', type: 'img', effect: 'in'}},
  {dur: 33, kind: 'text', text: 'More than|you started with.', highlights: ['more'], size: 84},
  {dur: 124, kind: 'text', enter: 'slideL', text: 'Two investors.|Same market.', highlights: ['same'], size: 88, media: {src: 'drag/p_twopaths.jpg', type: 'img', effect: 'in'}},
  {dur: 57, kind: 'stat', stat: {pre: 'The same', value: 7, suffix: '%', post: 'return'}, media: {src: 'drag/v_chart.mp4', type: 'video', effect: 'in'}},
  {dur: 70, kind: 'stat', stat: {pre: 'One pays', value: 0.05, decimals: 2, suffix: '%'}, media: {src: 'drag/p_singlecoin.jpg', type: 'img', effect: 'in'}},
  {dur: 79, kind: 'stat', stat: {pre: 'The other pays', value: 1, suffix: '%'}, media: {src: 'drag/p_skim.jpg', type: 'img', effect: 'in'}},
  {dur: 86, kind: 'stat', stat: {pre: 'Low-cost fund', prefix: '$', value: 755, suffix: 'K'}, media: {src: 'drag/p_tallstack.jpg', type: 'img', effect: 'in'}},
  {dur: 86, kind: 'stat', stat: {pre: 'The 1% fund', prefix: '$', value: 574, suffix: 'K'}, media: {src: 'drag/p_shortstack.jpg', type: 'img', effect: 'in'}},
  {dur: 52, kind: 'text', text: 'On identical|market exposure.', highlights: ['identical'], size: 78, media: {src: 'drag/p_scale.jpg', type: 'img', effect: 'in'}},
  {dur: 40, kind: 'text', text: 'The fee isn\'t|taken once.', highlights: ['once'], size: 82, media: {src: 'drag/p_calendar.jpg', type: 'img', effect: 'in'}},
  {dur: 63, kind: 'text', enter: 'slideL', text: 'It\'s charged|every year.', highlights: ['every'], size: 84, media: {src: 'drag/v_clock.mp4', type: 'video', effect: 'in'}},
  {dur: 101, kind: 'text', enter: 'zoom', text: 'On the growth|it prevented.', highlights: ['prevented'], size: 82, media: {src: 'drag/p_sprout.jpg', type: 'img', effect: 'in'}},
  {dur: 41, kind: 'text', text: 'It won\'t|even win.', highlights: ['win'], size: 92, media: {src: 'drag/p_race.jpg', type: 'img', effect: 'in'}},
  {dur: 79, kind: 'stat', stat: {pre: 'Active funds trailing', value: 90, suffix: '%', bar: 90}, media: {src: 'drag/p_bars.jpg', type: 'img', effect: 'in'}},
  {dur: 45, kind: 'text', text: 'Over|15 years.', highlights: ['15'], size: 98, media: {src: 'drag/p_hourglass.jpg', type: 'img', effect: 'in'}},
  {dur: 78, kind: 'text', text: 'A premium|to underperform.', highlights: ['underperform'], size: 82},
  {dur: 64, kind: 'impact', text: 'The fee is certain.|The upside isn\'t.', redBg: true},
  {dur: 71, kind: 'text', text: 'One thing|you control.', highlights: ['control'], size: 90, media: {src: 'drag/p_dial.jpg', type: 'img', effect: 'in'}},
  {dur: 78, kind: 'text', enter: 'slideL', text: 'Over decades,|a fortune.', highlights: ['fortune'], size: 86, media: {src: 'drag/p_nestegg.jpg', type: 'img', effect: 'in'}},
  {dur: 77, kind: 'text', text: 'The fee is|guaranteed.', highlights: ['guaranteed'], size: 86, media: {src: 'drag/p_stamp.jpg', type: 'img', effect: 'in'}},
  {dur: 73, kind: 'text', enter: 'zoom', text: 'What is that 1%|actually buying?', highlights: ['buying?'], size: 78, media: {src: 'drag/v_review.mp4', type: 'video', effect: 'in'}},
];
...
// 107), so every cue below is already on the post-pause timeline.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 143, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 219, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 432, src: 'media/sfx_impact.mp3', vol: 0.45},
  {at: 638, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 724, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 1107, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 1309, src: 'media/sfx_impact.mp3', vol: 0.6},
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The certain drag</span>
      </div>
...
// ---------------------------------------------------------------------------
export const DragReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('drag/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('drag/music.mp3')}
