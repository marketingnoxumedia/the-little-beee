...
// ---------------------------------------------------------------------------
// "The endless payment" reel — leasing a car forever vs buying and holding one (a
// punchy-hook take on the topic that is ALSO the existing LeaseReel — publish only one
// of the two, or space them far apart). Two people drive the same car for the same ten
// years; you'd think the cost comes out even. One ends the decade owning nothing and
// still paying; the other stopped paying years ago. The only difference is how they hold
// it — one leases continuously (always a new car, but the payment never stops), the
// other buys once (more up front, then the loan ends). After ten years the shapes split:
// the perpetual leaser still pays and owns nothing, the owner is paid off with the cost
// trending toward zero. Leasing buys the newest car and an endless payment; over a long
// horizon, holding costs less, and the longer the car is kept, the wider the gap. No
// count-up stats (this is a shape/comparison, not a figures reel — the row carries no
// hero dollar number), a short ~1.5s pause after the hook (house convention), four black
// beats, one red "leasing buys an endless payment" `impact` card near the tail, and a
// question close on footage (a car at dusk) rather than the red card. Durations on exact
// spoken-word timestamps (ElevenLabs alignment) at +6% pace. Total 1360 frames = ~45.3s.
// `enter` adds a transition on some cuts. Every scene uses a distinct background sourced
// fresh for THIS reel (public/payment/) — no footage shared with LeaseReel or any other
// reel (hard rule). FLAG: general characterization — leasing-forever vs buy-and-hold cost
// direction is the widely-cited result, but terms and rates move the size, not the
// direction; verify for specific numbers before publishing; no dollar figures are shown.
// Imagery is generic (no car-brand badges/logos, no dealership brands, no plates, no flags).
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 83, kind: 'hook', text: 'Two people, same car,|same ten years.', kicker: 'The endless payment', highlights: ['ten'], size: 84, media: {src: 'payment/v_hook.mp4', type: 'video', effect: 'in'}},
  {dur: 75, kind: 'text', enter: 'slideL', text: 'You\'d think it costs|about the same.', highlights: ['same'], size: 88, media: {src: 'payment/p_twocars.jpg', type: 'img', effect: 'in'}},
  {dur: 68, kind: 'text', text: 'One owns nothing,|still paying.', highlights: ['nothing'], size: 90, media: {src: 'payment/p_paying.jpg', type: 'img', effect: 'in'}},
  // Short ~1.5s pause after the hook (house convention): 45 frames of silence are spliced
  // into the VO right after "...paying years ago", and this beat holds the hook through it.
  {dur: 94, kind: 'text', text: 'The other stopped|paying years ago.', highlights: ['stopped'], size: 88, media: {src: 'payment/p_paidoff.jpg', type: 'img', effect: 'in'}},
  {dur: 58, kind: 'text', text: 'The only difference:|how they hold it.', highlights: ['hold'], size: 86},
  {dur: 58, kind: 'text', enter: 'slideL', text: 'One leases,|over and over.', highlights: ['leases'], size: 92, media: {src: 'payment/p_dealer.jpg', type: 'img', effect: 'in'}},
  {dur: 75, kind: 'text', text: 'The other buys once,|and keeps it.', highlights: ['keeps'], size: 88, media: {src: 'payment/p_keep.jpg', type: 'img', effect: 'in'}},
  {dur: 49, kind: 'text', enter: 'slideL', text: 'Lease forever —|always a new car.', highlights: ['new'], size: 86, media: {src: 'payment/v_newcars.mp4', type: 'video', effect: 'in'}},
  {dur: 66, kind: 'text', text: 'But the payment|never stops.', highlights: ['never'], size: 92},
  {dur: 32, kind: 'text', text: 'Buy: more|up front.', highlights: ['front'], size: 96, media: {src: 'payment/p_upfront.jpg', type: 'img', effect: 'in'}},
  {dur: 78, kind: 'text', text: 'Then the loan|simply ends.', highlights: ['ends'], size: 92, media: {src: 'payment/p_loanend.jpg', type: 'img', effect: 'in'}},
  {dur: 45, kind: 'text', text: 'After ten years,|the shapes split.', highlights: ['split'], size: 88},
  {dur: 93, kind: 'text', enter: 'slideL', text: 'The leaser still pays.|Owns nothing.', highlights: ['nothing'], size: 86, media: {src: 'payment/p_stillpaying.jpg', type: 'img', effect: 'in'}},
  {dur: 61, kind: 'text', text: 'The owner is paid off,|still driving.', highlights: ['paid'], size: 84, media: {src: 'payment/p_owner.jpg', type: 'img', effect: 'in'}},
  {dur: 96, kind: 'text', text: 'The cost trending|toward zero.', highlights: ['zero'], size: 92, media: {src: 'payment/p_money.jpg', type: 'img', effect: 'in'}},
  {dur: 71, kind: 'impact', text: 'Leasing buys|an endless payment.', redBg: true},
  {dur: 92, kind: 'text', enter: 'slideL', text: 'Over the long haul,|holding costs less.', highlights: ['less'], size: 84, media: {src: 'payment/v_road.mp4', type: 'video', effect: 'in'}},
  {dur: 41, kind: 'text', text: 'The longer you keep it,|the wider the gap.', highlights: ['wider'], size: 82, media: {src: 'payment/p_gap.jpg', type: 'img', effect: 'in'}},
  {dur: 62, kind: 'text', text: 'Same car.|Same years.', highlights: ['years'], size: 100},
  {dur: 63, kind: 'text', enter: 'zoom', text: 'What was that endless|payment buying?', highlights: ['endless'], size: 86, media: {src: 'payment/v_close.mp4', type: 'video', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
// NOTE: a 45-frame (~1.5s) pause is spliced after the hook (at frame ~261), so cues at
// or after the hook/reveal boundary are +45 from the pre-pause timeline.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 83, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 320, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 560, src: 'media/sfx_impact.mp3', vol: 0.45},
  {at: 736, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 874, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 935, src: 'media/sfx_impact.mp3', vol: 0.72},
  {at: 1297, src: 'media/sfx_whoosh.mp3', vol: 0.4},
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The endless payment</span>
      </div>
...
// ---------------------------------------------------------------------------
export const PaymentReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('payment/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('payment/music.mp3')}
