...
// ---------------------------------------------------------------------------
// "Income vs. wealth" reel (CeilingReel) — a big salary is income; what you keep is wealth.
// You think a big salary means you're rich? Plenty of people earning $300K a year have almost
// nothing saved — income and wealth aren't the same thing. Income is visible (the car, the
// title, the address) while net worth stays invisible, so the paycheck becomes the scoreboard.
// But net worth tracks the savings rate far more than the salary; studies of real millionaires
// keep finding ordinary incomes, high savings, and decades of investing, and lifestyle expands
// to fill income, so a raise without more saving leaves net worth flat. Income is what flows in;
// net worth is what you keep. Someone on $300K who spends it all builds nothing; someone on $90K
// who keeps a fifth compounds into real wealth — same effort, decades apart. The decisive number
// was never the paycheck; income sets the ceiling, and the savings rate decides how much of it
// becomes wealth. Numbers-driven: one count-up `stat` beat ($300k) plus the $300k-vs-$90k
// contrast. A short ~1.5s pause after the hook (house convention), black beats for the
// punch-lines, one red "it was never the paycheck" `impact` card as the mid-tail thesis, and a
// rhetorical question close ("what were you measuring?") on footage rather than the red card.
// Durations on exact spoken-word timestamps (ElevenLabs alignment) at +6% pace. Total 1582
// frames = ~52.7s. `enter` adds a transition on some cuts. Every scene uses a distinct
// background sourced fresh for THIS reel (public/ceiling/) — no footage shared with any other
// reel (hard rule). NOTE: this is the SAME topic as the existing KeepReel ("Income vs. wealth")
// — publish only one, or run this as a deliberate A/B / refreshed take. FLAG: illustrative — the
...
// (ordinary incomes + high savings), worth a light check before publishing. Imagery is generic
// (no car badges/logos, no employer names on pay stubs, no readable plates or addresses).
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 146, kind: 'hook', text: 'Big salary|means rich?', kicker: 'What you keep', highlights: ['rich?'], size: 92, media: {src: 'ceiling/v_hook.mp4', type: 'video', effect: 'in'}},
  // Short ~1.5s pause after the hook (house convention): 45 frames of silence are spliced
  // into the VO right after "...means you're rich?", and this beat holds through it.
  {dur: 54, kind: 'stat', stat: {pre: 'EARNING', prefix: '$', value: 300000, post: 'A YEAR'}, media: {src: 'ceiling/p_paycheck.jpg', type: 'img', effect: 'in'}},
  {dur: 62, kind: 'text', text: 'Almost nothing|saved.', highlights: ['nothing'], size: 94, media: {src: 'ceiling/p_empty.jpg', type: 'img', effect: 'in'}},
  {dur: 28, kind: 'text', text: 'Income isn\'t|wealth.', highlights: ['wealth'], size: 96},
  {dur: 32, kind: 'text', text: 'Income is|visible.', highlights: ['visible'], size: 96},
  {dur: 71, kind: 'lines', text: 'The car.|The title.|The address.', highlights: ['address'], reveal: [0, 24, 48], media: {src: 'ceiling/p_house.jpg', type: 'img', effect: 'in'}},
  {dur: 60, kind: 'text', text: 'Net worth stays|invisible.', highlights: ['invisible'], size: 90},
  {dur: 61, kind: 'text', enter: 'slideL', text: 'The paycheck is|the scoreboard.', highlights: ['scoreboard'], size: 86, media: {src: 'ceiling/p_car.jpg', type: 'img', effect: 'in'}},
  {dur: 126, kind: 'text', text: 'It tracks the|savings rate.', highlights: ['savings'], size: 90, media: {src: 'ceiling/p_save.jpg', type: 'img', effect: 'in'}},
  {dur: 117, kind: 'lines', text: 'Ordinary incomes.|High savings.|Decades in.', highlights: ['savings'], reveal: [0, 39, 78], media: {src: 'ceiling/p_invest.jpg', type: 'img', effect: 'in'}},
  {dur: 105, kind: 'text', enter: 'slideL', text: 'Lifestyle expands|to fill it.', highlights: ['expands'], size: 88, media: {src: 'ceiling/v_lux.mp4', type: 'video', effect: 'in'}},
  {dur: 88, kind: 'text', text: 'A raise leaves|net worth flat.', highlights: ['flat'], size: 88},
  {dur: 75, kind: 'text', text: 'Income flows in.|You keep net worth.', highlights: ['keep'], size: 84, media: {src: 'ceiling/p_cash.jpg', type: 'img', effect: 'in'}},
  {dur: 90, kind: 'text', text: '$300K, spends it all.|Builds nothing.', highlights: ['nothing'], size: 82, media: {src: 'ceiling/p_worker.jpg', type: 'img', effect: 'in'}},
  {dur: 94, kind: 'text', enter: 'slideL', text: '$90K, keeps a fifth.|Real wealth.', highlights: ['wealth'], size: 82, media: {src: 'ceiling/v_grow.mp4', type: 'video', effect: 'in'}},
  {dur: 63, kind: 'text', text: 'Same effort.|Decades apart.', highlights: ['decades'], size: 88, media: {src: 'ceiling/p_wealth.jpg', type: 'img', effect: 'in'}},
  {dur: 45, kind: 'impact', text: 'It was never|the paycheck.', redBg: true},
  {dur: 50, kind: 'text', text: 'Income sets|the ceiling.', highlights: ['ceiling'], size: 90},
  {dur: 75, kind: 'text', text: 'The savings rate|decides.', highlights: ['decides'], size: 90},
  {dur: 84, kind: 'text', enter: 'slideL', text: 'Two earn the same.|Worlds apart.', highlights: ['apart'], size: 88},
  {dur: 56, kind: 'text', enter: 'zoom', text: 'What were you|measuring?', highlights: ['measuring?'], size: 90, media: {src: 'ceiling/v_close.mp4', type: 'video', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
// NOTE: a 45-frame (~1.5s) pause is spliced after the hook (at frame ~101), so cues from the
// $300k reveal (frame 200) on are already on the post-pause timeline.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 200, src: 'media/sfx_impact.mp3', vol: 0.5},   // $300k stat (reveal after the pause)
  {at: 453, src: 'media/sfx_whoosh.mp3', vol: 0.4},   // "the paycheck is the scoreboard"
  {at: 1025, src: 'media/sfx_whoosh.mp3', vol: 0.4},  // "$300K spends it all"
  {at: 1115, src: 'media/sfx_impact.mp3', vol: 0.5},  // "$90K keeps a fifth / real wealth"
  {at: 1272, src: 'media/sfx_impact.mp3', vol: 0.72}, // red thesis card
  {at: 1526, src: 'media/sfx_whoosh.mp3', vol: 0.4},  // close
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>What you keep</span>
      </div>
...
// ---------------------------------------------------------------------------
export const CeilingReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('ceiling/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('ceiling/music.mp3')}
