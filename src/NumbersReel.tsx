...
// ---------------------------------------------------------------------------
// "Run the numbers" reel (NumbersReel) — the rent-vs-buy myth. "Renting is throwing money
// away" is one of the most expensive things people believe; sometimes buying is the worse
// move — it depends on numbers most people never run. The belief is easy to hold (rent leaves
// nothing to point to, a mortgage feels like forced saving), but buying carries costs the
// slogan ignores — interest, property taxes, maintenance, insurance, fees — and the down
// payment has an opportunity cost (that cash could have been invested). Whether buying wins
// depends on how long you stay, the local price-to-rent ratio, and what the cash would
// otherwise earn. Owners build equity but tie up capital; renters keep flexibility and invest
// the difference. The same house can favor buying in one city and renting in another. The
// honest answer is a calculation, not a slogan — change the inputs and it flips. This is a
// CONCEPTUAL reel (no count-up stats): two `lines` beats carry the cost list and the three
// inputs. A short ~1.5s pause after the hook (house convention), five black beats for the
// punch-lines/lists, one red "a calculation, not a slogan" `impact` card as the mid-tail
// thesis, and a rhetorical question close on footage (a residential street at dusk) rather
// than the red card. Durations on exact spoken-word timestamps (ElevenLabs alignment) at +6%
// pace. Total 1519 frames = ~50.6s. `enter` adds a transition on some cuts. Every scene uses a
// distinct background sourced fresh for THIS reel (public/numbers/) — no footage shared with
// any other reel (hard rule). NOTE: this is the SAME topic as the existing RentReel ("Rent
// vs. buy — it depends") — publish only one, or run this as a deliberate A/B / refreshed take.
// FLAG: no hard figures are claimed; the rent-vs-buy conclusion is a general characterization
// (the answer genuinely depends on the inputs). Imagery is generic (no realtor/agency names or
// logos, no lender brands, no readable addresses or plates).
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 103, kind: 'hook', text: 'Renting throws|money away.', kicker: 'Run the numbers', highlights: ['away'], size: 84, media: {src: 'numbers/v_hook.mp4', type: 'video', effect: 'in'}},
  // Short ~1.5s pause after the hook (house convention): 45 frames of silence are spliced
  // into the VO right after "...things people believe?", and this beat holds through it.
  {dur: 116, kind: 'text', text: 'One expensive|belief.', highlights: ['expensive'], size: 92, media: {src: 'numbers/p_forsale.jpg', type: 'img', effect: 'in'}},
  {dur: 64, kind: 'text', text: 'Sometimes buying|is worse.', highlights: ['worse'], size: 90, media: {src: 'numbers/p_mortgage.jpg', type: 'img', effect: 'in'}},
  {dur: 69, kind: 'text', enter: 'slideL', text: 'Numbers you|never run.', highlights: ['numbers'], size: 92, media: {src: 'numbers/p_calc.jpg', type: 'img', effect: 'in'}},
  {dur: 62, kind: 'text', text: 'Rent leaves|nothing behind.', highlights: ['nothing'], size: 90, media: {src: 'numbers/p_apartment.jpg', type: 'img', effect: 'in'}},
  {dur: 60, kind: 'text', text: 'A mortgage feels|like saving.', highlights: ['saving'], size: 92},
  {dur: 51, kind: 'text', text: 'But buying has|hidden costs.', highlights: ['costs'], size: 92, media: {src: 'numbers/p_upkeep.jpg', type: 'img', effect: 'in'}},
  {dur: 137, kind: 'lines', text: 'Interest|Property taxes|Maintenance|Insurance|Fees', highlights: ['Fees'], reveal: [0, 28, 56, 84, 112]},
  {dur: 40, kind: 'text', text: 'The down payment|has a cost.', highlights: ['cost'], size: 90, media: {src: 'numbers/p_downpayment.jpg', type: 'img', effect: 'in'}},
  {dur: 77, kind: 'text', enter: 'slideL', text: 'That cash could|be invested.', highlights: ['invested'], size: 90, media: {src: 'numbers/p_invest.jpg', type: 'img', effect: 'in'}},
  {dur: 156, kind: 'lines', text: 'How long you stay|Price-to-rent|What the cash earns', highlights: ['pricetorent'], reveal: [0, 55, 110]},
  {dur: 28, kind: 'text', text: 'It isn\'t|universal.', highlights: ['universal'], size: 96},
  {dur: 69, kind: 'text', text: 'Owners build|equity.', highlights: ['equity'], size: 94, media: {src: 'numbers/p_equity.jpg', type: 'img', effect: 'in'}},
  {dur: 35, kind: 'text', text: 'Renters keep|flexibility.', highlights: ['flexibility'], size: 92, media: {src: 'numbers/p_boxes.jpg', type: 'img', effect: 'in'}},
  {dur: 80, kind: 'text', enter: 'slideL', text: 'And invest|the difference.', highlights: ['invest'], size: 92, media: {src: 'numbers/v_city.mp4', type: 'video', effect: 'in'}},
  {dur: 57, kind: 'text', text: 'Buy one city.|Rent another.', highlights: ['another'], size: 90, media: {src: 'numbers/p_neighborhood.jpg', type: 'img', effect: 'in'}},
  {dur: 108, kind: 'text', enter: 'slideL', text: 'The honest answer|is a calculation.', highlights: ['calculation'], size: 82, media: {src: 'numbers/v_keys.mp4', type: 'video', effect: 'in'}},
  {dur: 53, kind: 'text', text: 'Change the inputs —|it flips.', highlights: ['flips'], size: 90},
  {dur: 102, kind: 'impact', text: 'A calculation,|not a slogan.', redBg: true},
  {dur: 52, kind: 'text', enter: 'zoom', text: 'Which way do the|numbers tip?', highlights: ['tip?'], size: 90, media: {src: 'numbers/v_close.mp4', type: 'video', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
// NOTE: a 45-frame (~1.5s) pause is spliced after the hook (at frame ~157), so cues at or
// after the reveal (frame 219) are already on the post-pause timeline.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 219, src: 'media/sfx_whoosh.mp3', vol: 0.4},   // reveal after the pause
  {at: 525, src: 'media/sfx_whoosh.mp3', vol: 0.4},   // "hidden costs" into the list
  {at: 662, src: 'media/sfx_impact.mp3', vol: 0.5},   // the cost list
  {at: 779, src: 'media/sfx_impact.mp3', vol: 0.45},  // the three inputs
  {at: 1204, src: 'media/sfx_whoosh.mp3', vol: 0.4},  // "the honest answer" turn
  {at: 1365, src: 'media/sfx_impact.mp3', vol: 0.72}, // red thesis card
  {at: 1467, src: 'media/sfx_whoosh.mp3', vol: 0.4},  // close
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Run the numbers</span>
      </div>
...
// ---------------------------------------------------------------------------
export const NumbersReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('numbers/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('numbers/music.mp3')}
