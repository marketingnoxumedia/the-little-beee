...
// ---------------------------------------------------------------------------
// "Boring is the moat" reel — self-storage as a boring-but-highly-profitable business
// (a fresh, punchier take on the topic that is ALSO the repo's flagship StorageVideo /
// StorageUnit — publish only one of the two, or space them far apart). A business with
// almost no staff pulls in ~$4.8B a year, and you drive past it every week without a
// glance: self-storage. Metal units on cheap land, run by the customers themselves; the
// company owns the boxes and collects the rent. The moat is what it doesn't need — no
// inventory, almost no staff, cheap upkeep — and once your things are inside, moving out
// costs a weekend and a truck, so the small annual rent bumps just get paid. Recessions
// don't empty the units (moves, divorces, downsizing are what a downturn produces), so
// rent rises automatically and customers rarely leave — which is how metal boxes keep
// ~79 cents of every dollar at the facility level and land ~37% net margin. Boring is
// the moat. Three count-up stats ($4.8B / 79% / 37%), a short ~1.5s pause after the
// hook (the hook holds while silence sits before the "It's self-storage" reveal — house
// convention), seven black beats (punch-lines and lists in the dark), one red "Boring is
// the moat" `impact` card near the tail, and a question close that resolves on footage
// (a shed at dusk) rather than the red card. Durations on exact spoken-word timestamps
// (ElevenLabs alignment) at +6% pace. Total 1946 frames = ~64.9s. `enter` adds a
// transition on some cuts. Every scene uses a distinct background sourced fresh for THIS
// reel (public/shed/) — no footage shared with StorageVideo or any other reel (hard
// rule). FLAG: the ~$4.8B revenue / ~79% facility-level / ~37% net-margin figures echo a
// large public self-storage operator but are illustrative round numbers — verify before
// publishing. Imagery is generic (no storage-brand signage, no truck brands, no flags).
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 98, kind: 'hook', text: 'How does a business|with no staff...', kicker: 'Boring is the moat', highlights: ['staff'], size: 84, media: {src: 'shed/v_hook.mp4', type: 'video', effect: 'in'}},
  {dur: 115, kind: 'stat', stat: {pre: 'With almost no staff', prefix: '$', value: 4.8, decimals: 1, suffix: 'B', post: 'A year'}, media: {src: 'shed/p_units.jpg', type: 'img', effect: 'in'}},
  // Short ~1.5s pause after the hook (house convention): 45 frames of silence are
  // spliced into the VO right after "...never look twice", and this beat holds the hook
  // on screen through it before the "It's self-storage" reveal.
  {dur: 83, kind: 'text', text: 'You drive past it,|never look twice.', highlights: ['never'], size: 84, media: {src: 'shed/v_driveby.mp4', type: 'video', effect: 'in'}},
  {dur: 44, kind: 'text', enter: 'slideL', text: 'It\'s|self-storage.', highlights: ['selfstorage'], size: 100, media: {src: 'shed/p_facility.jpg', type: 'img', effect: 'in'}},
  {dur: 90, kind: 'text', text: 'Metal units|on cheap land.', highlights: ['cheap'], size: 90, media: {src: 'shed/p_metaldoors.jpg', type: 'img', effect: 'in'}},
  {dur: 40, kind: 'text', text: 'Run by the customers|themselves.', highlights: ['customers'], size: 84, media: {src: 'shed/p_selfaccess.jpg', type: 'img', effect: 'in'}},
  {dur: 63, kind: 'text', text: 'It owns the boxes,|collects the rent.', highlights: ['rent'], size: 82, media: {src: 'shed/p_padlock.jpg', type: 'img', effect: 'in'}},
  {dur: 42, kind: 'text', text: 'The moat is what|it doesn\'t need.', highlights: ['moat'], size: 84},
  {dur: 134, kind: 'lines', text: 'No inventory.|Almost no staff.|Simple upkeep.', highlights: ['staff']},
  {dur: 77, kind: 'text', enter: 'slideL', text: 'Once your things|are inside...', highlights: ['inside'], size: 88, media: {src: 'shed/p_boxes.jpg', type: 'img', effect: 'in'}},
  {dur: 47, kind: 'text', text: 'It costs a weekend|and a truck.', highlights: ['weekend'], size: 86, media: {src: 'shed/v_truck.mp4', type: 'video', effect: 'in'}},
  {dur: 87, kind: 'text', text: 'The rent ticks up.|You just pay it.', highlights: ['pay'], size: 88},
  {dur: 61, kind: 'text', text: 'Recessions don\'t|empty the units.', highlights: ['recessions'], size: 84, media: {src: 'shed/p_recession.jpg', type: 'img', effect: 'in'}},
  {dur: 112, kind: 'lines', text: 'Moves.|Divorces.|Downsizing.', highlights: ['divorces']},
  {dur: 46, kind: 'text', text: 'What a downturn|produces.', highlights: ['downturn'], size: 90, media: {src: 'shed/p_moving.jpg', type: 'img', effect: 'in'}},
  {dur: 50, kind: 'text', text: 'The rent rises|automatically.', highlights: ['automatically'], size: 84},
  {dur: 88, kind: 'text', enter: 'slideL', text: 'And the customers|rarely leave.', highlights: ['rarely'], size: 88, media: {src: 'shed/p_longterm.jpg', type: 'img', effect: 'in'}},
  {dur: 105, kind: 'stat', stat: {pre: 'Kept per dollar', value: 79, suffix: '%', post: 'At the facility', bar: 79}, media: {src: 'shed/p_cash.jpg', type: 'img', effect: 'in'}},
  {dur: 85, kind: 'stat', stat: {pre: 'Net margin', value: 37, suffix: '%'}, media: {src: 'shed/p_ledger.jpg', type: 'img', effect: 'in'}},
  {dur: 83, kind: 'text', text: 'The most boring|real estate there is.', highlights: ['boring'], size: 82, media: {src: 'shed/p_boring.jpg', type: 'img', effect: 'in'}},
  {dur: 29, kind: 'text', text: 'Quietly, the most|profitable.', highlights: ['profitable'], size: 88},
  {dur: 32, kind: 'impact', text: 'Boring is|the moat.', redBg: true},
  {dur: 108, kind: 'lines', text: 'No brand.|No buzz.|Nothing exciting.', highlights: ['brand']},
  {dur: 87, kind: 'text', enter: 'slideL', text: 'Rent that goes up.|Customers who stay.', highlights: ['stay'], size: 84, media: {src: 'shed/p_gate.jpg', type: 'img', effect: 'in'}},
  {dur: 73, kind: 'text', text: 'What\'s the most profitable|thing you\'ll drive past?', highlights: ['profitable'], size: 80, media: {src: 'shed/v_road.mp4', type: 'video', effect: 'in'}},
  {dur: 67, kind: 'text', enter: 'zoom', text: 'A shed you forgot|you were paying for.', highlights: ['shed'], size: 88, media: {src: 'shed/p_shed.jpg', type: 'img', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
// NOTE: a 45-frame (~1.5s) pause is spliced after the hook (at frame ~228), so cues at
// or after the hook/reveal boundary are +45 from the pre-pause timeline.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 98, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 296, src: 'media/sfx_whoosh.mp3', vol: 0.45},
  {at: 575, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 1189, src: 'media/sfx_impact.mp3', vol: 0.55},
  {at: 1277, src: 'media/sfx_impact.mp3', vol: 0.55},
  {at: 1579, src: 'media/sfx_impact.mp3', vol: 0.72},
  {at: 1879, src: 'media/sfx_whoosh.mp3', vol: 0.4},
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Boring is the moat</span>
      </div>
...
// ---------------------------------------------------------------------------
export const ShedReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('shed/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('shed/music.mp3')}
