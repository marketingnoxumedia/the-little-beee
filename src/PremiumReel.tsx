...
// ---------------------------------------------------------------------------
// "What the label buys" reel (PremiumReel) — the store-label vs name-brand premium myth-bust,
// told as the A/B twin of the existing BrandReel ("Paying for the name"), which opens and closes
// declaratively. You think the name brand is the better product — so why is the store label beside
// it often made in the same factory, to the same standard, for about a third less? The premium
// feels earned (a trusted name, a higher price; expensive reads as better), but often that store
// label is made to the same spec, sometimes on the same production line, so part of the premium
// pays for the label, not the contents — for shelf space and ads, not what is inside. It is not
// true everywhere, which is the point: sometimes the premium is a real difference in quality,
// sometimes it is not, and the only way to know is to compare the actual items, not the names. The
// honest rule is not "always buy cheap" — the label alone tells you nothing about what is inside;
// where the specs match, the extra just buys the name, and where they do not, it can be worth it.
// Question hook ("...made in the same factory, to the same standard, for a third less?") and a
// rhetorical question close ("what are you actually paying extra for?"); a ~1.5s post-hook pause
// (the hook holds through the silence before the reveal — house convention); one red "The label,
// not the contents." `impact` card as the mid-tail thesis, and the question close on footage.
// Media-dense: 18 of 21 beats carry footage, with only 2 flat-black beats plus the one red card.
// Durations on exact ElevenLabs spoken-word timestamps at +6% pace; music normalized and mixed at
// 0.17. Total 1461 frames = ~48.7s. Entirely its own fresh footage under public/premium/ — no
// asset shared with BrandReel, LabelReel, or any other reel. Imagery is deliberately generic — no
// legible product/store/ad brand logos, wordmarks, or readable labels (the whole point of the
// piece is the label, so real brands are kept out of frame). **FLAG — qualitative claim:** the
// "same factory / same standard / ~a third less" framing is broadly true in many private-label
// categories but varies by product and retailer, and no specific brand or figure is asserted.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 87, kind: 'hook', text: 'You think the brand|is better.', kicker: 'What the label buys', highlights: ['brand'], size: 84, media: {src: 'premium/v_shelf.mp4', type: 'video', effect: 'in'}},
  {dur: 64, kind: 'text', enter: 'slideL', text: 'So why is the|store label—', highlights: ['store'], size: 84, media: {src: 'premium/p_twoproducts.jpg', type: 'img', effect: 'in'}},
  {dur: 63, kind: 'text', text: 'made in the|same factory,', highlights: ['same'], size: 88, media: {src: 'premium/v_factory.mp4', type: 'video', effect: 'in'}},
  // Short ~1.5s pause after the hook (house convention): 45 frames of silence are spliced into the
  // VO right after "...for a third less?", and this black beat holds through it (30 -> 75).
  {dur: 75, kind: 'text', text: 'for a|third less?', highlights: ['third'], size: 96},
  {dur: 41, kind: 'text', text: 'The premium|feels earned.', highlights: ['premium'], size: 92, media: {src: 'premium/p_pricetag.jpg', type: 'img', effect: 'in'}},
  {dur: 76, kind: 'text', text: 'A trusted name.|A higher price.', highlights: ['trusted'], size: 86, media: {src: 'premium/p_fancy.jpg', type: 'img', effect: 'in'}},
  {dur: 84, kind: 'text', enter: 'slideL', text: 'Expensive reads|as better.', highlights: ['expensive'], size: 86, media: {src: 'premium/p_shopper.jpg', type: 'img', effect: 'in'}},
  {dur: 45, kind: 'text', text: 'Made to the|same spec.', highlights: ['spec'], size: 92, media: {src: 'premium/v_line.mp4', type: 'video', effect: 'in'}},
  {dur: 93, kind: 'text', text: 'Sometimes the|same line.', highlights: ['line'], size: 90, media: {src: 'premium/p_conveyor.jpg', type: 'img', effect: 'in'}},
  {dur: 37, kind: 'impact', text: 'The label,|not the contents.', redBg: true},
  {dur: 86, kind: 'text', enter: 'slideR', text: 'Shelf space|and ads.', highlights: ['ads'], size: 90, media: {src: 'premium/p_ads.jpg', type: 'img', effect: 'in'}},
  {dur: 68, kind: 'text', text: "Not true|everywhere.", highlights: ['everywhere'], size: 90, media: {src: 'premium/p_shelf2.jpg', type: 'img', effect: 'in'}},
  {dur: 81, kind: 'text', text: "Sometimes it's|real quality.", highlights: ['real'], size: 88, media: {src: 'premium/p_quality.jpg', type: 'img', effect: 'in'}},
  {dur: 59, kind: 'text', text: "Sometimes|it's not.", highlights: ['not'], size: 96},
  {dur: 115, kind: 'text', enter: 'slideL', text: 'Compare the items,|not the names.', highlights: ['compare'], size: 84, media: {src: 'premium/v_compare.mp4', type: 'video', effect: 'in'}},
  {dur: 70, kind: 'text', text: "The rule isn't|always buy cheap.", highlights: ['cheap'], size: 84, media: {src: 'premium/p_cart.jpg', type: 'img', effect: 'in'}},
  {dur: 56, kind: 'text', text: 'The label tells you|nothing.', highlights: ['nothing'], size: 86, media: {src: 'premium/p_plain.jpg', type: 'img', effect: 'in'}},
  {dur: 107, kind: 'text', enter: 'slideL', text: 'Specs match?|The extra buys the name.', highlights: ['name'], size: 80, media: {src: 'premium/p_labels.jpg', type: 'img', effect: 'in'}},
  {dur: 62, kind: 'text', text: "Specs don't?|It can be worth it.", highlights: ['worth'], size: 82, media: {src: 'premium/p_premium2.jpg', type: 'img', effect: 'in'}},
  {dur: 49, kind: 'text', text: 'The cheaper one,|right beside it.', highlights: ['cheaper'], size: 84, media: {src: 'premium/v_shelf2.mp4', type: 'video', effect: 'in'}},
  {dur: 43, kind: 'text', enter: 'zoom', text: 'What are you|paying extra for?', highlights: ['extra'], size: 86, media: {src: 'premium/v_close.mp4', type: 'video', effect: 'in'}},
];
...
const SFX: SfxCue[] = [
  {at: 151, src: 'media/sfx_impact.mp3', vol: 0.5},   // 'same factory' hook punch
  {at: 289, src: 'media/sfx_whoosh.mp3', vol: 0.4},   // reveal after the pause
  {at: 628, src: 'media/sfx_impact.mp3', vol: 0.72},  // red 'The label, not the contents.' card
  {at: 959, src: 'media/sfx_whoosh.mp3', vol: 0.35},  // 'compare the items' cut
  {at: 1418, src: 'media/sfx_whoosh.mp3', vol: 0.4},  // question close
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>What the label buys</span>
      </div>
...
export const PremiumReel: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('premium/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('premium/music.mp3')}
